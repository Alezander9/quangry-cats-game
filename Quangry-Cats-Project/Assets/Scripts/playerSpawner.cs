...
public class playerSpawner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // if (!mycat.GetComponent<PlayerMovement>().isProjectile && (Input.GetKeyDown("q") || Input.GetKeyDown("4")))
        // {
        //     mycat.GetComponent<PlayerMovement>().launchSpeed += 10 * catrb.mass;
        // }

    }
    public void summonCatButton()
    {
        if (mycat.GetComponent<PlayerMovement>().isProjectile)
        {
            summonCat();
        }
    }
    ...
    void summonCat()
    {
        ...
        if (cats.Count > 0)
        {
            ...
            {
                Destroy(mycat);
                Destroy(GameObject.Find("catsprite"+(totalCats - cats.Count - 1).ToString()));
            }
            mycattype = cats[0];
            cats.RemoveAt(0);
            if (mycattype == 1)
